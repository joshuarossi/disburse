...
import { policyIntentValidator, policyFeeValidator, policyExecutionValidator } from './lib/spendingPolicyValidators';
import { ownerProposalValidator } from "./lib/ownerProposalValidator";
import { defineSchema, defineTable } from "convex/server";
import { accountingFact, accountingTreatment, accountKind, bookCurrency, journalLine } from './lib/accountingValidators';
import { v } from "convex/values";
import { payoutDetailsValidator, verificationMethodValidator } from './lib/recipientValidators';
import { sdnEntryFields, screeningInputValidator, screeningMatchValidator } from './lib/sanctionsValidators';
...
export default defineSchema({
  ...
  accountingProfiles: defineTable({ orgId: v.id('orgs'), currency: bookCurrency, bookName: v.string(),
    closedThrough: v.optional(v.string()), version: v.number(), nextJournal: v.number(), updatedAt: v.number(),
  }).index('by_org', ['orgId']),
  accountingAccounts: defineTable({ orgId: v.id('orgs'), externalId: v.string(), name: v.string(), kind: accountKind,
    active: v.boolean(), version: v.number(), updatedAt: v.number(),
  }).index('by_org', ['orgId']).index('by_external', ['orgId', 'externalId']),
  accountingMappings: defineTable({ orgId: v.id('orgs'), location: v.string(), accountId: v.id('accountingAccounts'), updatedAt: v.number(),
  }).index('by_location', ['orgId', 'location']),
  accountingMovements: defineTable({ orgId: v.id('orgs'), key: v.string(), entryId: v.id('accountingEntries'),
  }).index('by_movement', ['orgId', 'key']),
  accountingEntries: defineTable({ orgId: v.id('orgs'), journalNumber: v.string(), fact: accountingFact, currency: bookCurrency,
    treatment: accountingTreatment, postingDate: v.string(), assetBookValue: v.string(), obligationBookValue: v.optional(v.string()), advanceBookValue: v.optional(v.string()),
    bookReference: v.string(), externalName: v.optional(v.string()), valuationEvidence: v.string(), memo: v.string(),
    lines: v.array(journalLine), reviewedBy: v.id('users'), reviewedAt: v.number(), profileVersion: v.number(),
    state: v.union(v.literal('ready'), v.literal('exported'), v.literal('reconciled'), v.literal('void')),
    reversalOf: v.optional(v.id('accountingEntries')), replaces: v.optional(v.id('accountingEntries')),
    supersededBy: v.optional(v.id('accountingEntries')), correctionReason: v.optional(v.string()),
    pairedEntryId: v.optional(v.id('accountingEntries')),
    exportId: v.optional(v.id('accountingExports')), importedReference: v.optional(v.string()), reconciledAt: v.optional(v.number()),
  }).index('by_org_date', ['orgId', 'postingDate']).index('by_org_state', ['orgId', 'state'])
    .index('by_org_state_date', ['orgId', 'state', 'postingDate'])
    .index('by_movement', ['orgId', 'fact.key']),
  accountingExports: defineTable({ orgId: v.id('orgs'), requestId: v.string(), entryIds: v.array(v.id('accountingEntries')),
    currency: bookCurrency, environment: v.union(v.literal('production'), v.literal('test')),
    createdBy: v.id('users'), createdAt: v.number(), importedAt: v.optional(v.number()), importedReference: v.optional(v.string()),
  }).index('by_request', ['orgId', 'requestId']).index('by_org', ['orgId']),
  reportEntries: defineTable({ orgId: v.id('orgs'), sourceKey: v.string(), unclassified: v.boolean(), ...reportRowFields })
    .index('by_account_asset_time', ['orgId', 'safeId', 'assetId', 'createdAt'])
    .index('by_org_row', ['orgId', 'rowId'])
    .index('by_source', ['sourceKey']).index('by_org_environment_time', ['orgId', 'environment', 'createdAt'])
    .index('by_org_asset_time', ['orgId', 'assetId', 'createdAt'])
    .index('by_org_environment_token_time', ['orgId', 'environment', 'token', 'createdAt'])
    .index('by_org_environment_chain_time', ['orgId', 'environment', 'chainId', 'createdAt'])
    .index('by_org_unclassified_time', ['orgId', 'unclassified', 'createdAt'])
    .index('by_org_environment_recipient_time', ['orgId', 'environment', 'beneficiaryId', 'createdAt']),
  reportTotals: defineTable({ orgId: v.id('orgs'), dimension: v.string(), period: v.string(), ...reportAssetFields,
    inflowRaw: v.string(), outflowRaw: v.string(), count: v.number(),
  }).index('by_bucket', ['orgId', 'environment', 'dimension', 'period', 'assetId']),
  reportAssets: defineTable({ orgId: v.id('orgs'), unclassified: v.boolean(), ...reportAssetFields, firstAt: v.number(), lastAt: v.number(), count: v.number() })
    .index('by_asset', ['orgId', 'assetId']).index('by_org_environment', ['orgId', 'environment'])
    .index('by_org_unclassified', ['orgId', 'unclassified']).index('by_org_unclassified_address', ['orgId', 'unclassified', 'tokenAddress'])
  ...
  accountCancellations: defineTable({
    ...
    originalProposalId: v.id('accountProposals'), orgId: v.id('orgs'), safeId: v.id('safes'), chainId: v.number(), safeAddress: v.string(),
    originalHash: v.string(), nonce: v.number(), safeTxHash: v.string(), createdBy: v.id('users'), searchFromBlock: v.string(),
    executionFee: v.optional(policyFeeValidator),
    status: v.union(v.literal('pending'), v.literal('processing'), v.literal('applied'), v.literal('failed')),
  ...
  }).index('by_original', ['originalProposalId']).index('by_safe_status', ['safeId', 'status']).index('by_recovery', ['recoveryAt']),
  spendingPolicyChanges: defineTable({
    ...
    createdBy: v.id('users'), requestId: v.string(), intent: policyIntentValidator,
    executionFee: v.optional(policyFeeValidator), safeTxHash: v.string(),
    status: v.union(v.literal('pending'), v.literal('processing'), v.literal('applied'), v.literal('failed'), v.literal('cancelled')),
    execution: v.optional(policyExecutionValidator), recoveryAt: v.optional(v.number()), error: v.optional(v.string()),
    txHash: v.optional(v.string()), appliedAt: v.optional(v.number()), createdAt: v.number(), updatedAt: v.number(),
  ...
  receivables: defineTable({
    ...
    number: v.string(), normalizedNumber: v.string(), customerName: v.string(), customerEmail: v.optional(v.string()),
    description: v.string(), items: v.array(v.object({ description: v.string(), quantity: v.number(), unitPrice: v.string() })),
    token: v.string(), tokenAddress: v.string(), chainId: v.number(), treasury: v.string(), amount: v.string(), dueDate: v.number(),
    state: v.union(v.literal("draft"), v.literal("issued"), v.literal("void")),
    publicToken: v.optional(v.string()), factory: v.optional(v.string()), salt: v.optional(v.string()), receivingAddress: v.optional(v.string()),
    scanFromBlock: v.optional(v.string()), issuedAt: v.optional(v.number()), voidedAt: v.optional(v.number()),
    received: v.string(), forwarded: v.string(), revision: v.optional(v.number()), lastCheckedAt: v.optional(v.number()), nextScanAt: v.optional(v.number()), syncError: v.optional(v.string()),
    sweepState: v.optional(v.union(v.literal("submitting"), v.literal("submitted"), v.literal("attention"))),
    sweepProviderId: v.optional(v.string()), sweepAttemptAt: v.optional(v.number()), sweepError: v.optional(v.string()),
    createdAt: v.number(), updatedAt: v.number(),
  }).index("by_org", ["orgId"]).index("by_org_number", ["orgId", "normalizedNumber"]).index("by_public", ["publicToken"]).index("by_issued_scan", ["state", "nextScanAt"])
    .index('by_receiving_address', ['orgId', 'chainId', 'receivingAddress']),
  receivableEvents: defineTable({
    invoiceId: v.id("receivables"), orgId: v.id("orgs"), key: v.string(), kind: v.union(v.literal("received"), v.literal("forwarded")),
    amount: v.string(), txHash: v.string(), logIndex: v.number(), blockNumber: v.string(), blockHash: v.string(), recordedAt: v.number(),
    settledAt: v.optional(v.number()), fromAddress: v.optional(v.string()), toAddress: v.optional(v.string()),
  }).index("by_invoice", ["invoiceId"]).index("by_invoice_key", ["invoiceId", "key"]).index('by_org_time', ['orgId', 'recordedAt']),
  ...
  orgMemberships: defineTable({
    ...
    invitedBy: v.optional(v.id("users")), invitedAt: v.optional(v.number()), invitationExpiresAt: v.optional(v.number()),
  ...
  teamInvitations: defineTable({
    orgId: v.id("orgs"), email: v.string(), name: v.optional(v.string()),
    role: v.union(v.literal("admin"), v.literal("approver"), v.literal("initiator"), v.literal("clerk"), v.literal("viewer")),
    expectedWallet: v.optional(v.string()), tokenHash: v.string(),
    requestId: v.string(), requestHash: v.string(), createdBy: v.id("users"), createdAt: v.number(), expiresAt: v.number(),
    status: v.union(v.literal("pending"), v.literal("accepted"), v.literal("revoked")),
    deliveryId: v.optional(v.id("emailDeliveries")), acceptedBy: v.optional(v.id("users")), acceptedAt: v.optional(v.number()), revokedAt: v.optional(v.number()),
  ...
  beneficiaries: defineTable({
    ...
    payoutReviewStatus: v.optional(v.union(v.literal('unreviewed'), v.literal('approved'))),
  ...
  recipientImportBatches: defineTable({
    orgId:v.id('orgs'), requestId:v.string(), requestHash:v.string(), createdBy:v.id('users'), createdAt:v.number(),
    created:v.number(), updated:v.number(), reviewRequested:v.number(), recipientIds:v.array(v.id('beneficiaries')),
  }).index('by_org_request',['orgId','requestId']),

  recipientCollections: defineTable({
    orgId: v.id('orgs'), beneficiaryId: v.id('beneficiaries'), tokenHash: v.string(),
    createdBy: v.id('users'), createdAt: v.number(), expiresAt: v.number(),
    recipientFingerprint: v.string(), chainIds: v.array(v.number()),
    status: v.union(v.literal('requested'), v.literal('submitted'), v.literal('revoked')),
    revokedAt: v.optional(v.number()), submittedAt: v.optional(v.number()),
    submissionHash: v.optional(v.string()), changeId: v.optional(v.id('recipientChanges')),
  }).index('by_token_hash', ['tokenHash']).index('by_recipient', ['beneficiaryId']),

  recipientChanges: defineTable({
    orgId: v.id('orgs'), beneficiaryId: v.id('beneficiaries'), before: payoutDetailsValidator, proposed: payoutDetailsValidator,
    baseVersion: v.number(), requestedBy: v.id('users'), requestedAt: v.number(),
    collectionId: v.optional(v.id('recipientCollections')),
    status: v.union(v.literal('pending'), v.literal('approved'), v.literal('rejected'), v.literal('withdrawn')),
    reviewedBy: v.optional(v.id('users')), reviewedAt: v.optional(v.number()), verificationMethod: v.optional(verificationMethodValidator), reason: v.optional(v.string()),
  }).index('by_org_status', ['orgId', 'status']).index('by_recipient', ['beneficiaryId']),
  ...
  invoiceFiles: defineTable({
    orgId: v.id('orgs'), storageId: v.id('_storage'), invoiceId: v.optional(v.id('invoices')),
    requestId: v.string(), name: v.string(), contentType: v.string(), size: v.number(), sha256: v.string(),
    uploadedBy: v.id('users'), createdAt: v.number(), expiresAt: v.optional(v.number()),
  }).index('by_invoice', ['invoiceId']).index('by_request', ['orgId', 'requestId']).index('by_expiry', ['expiresAt']).index('by_user_unattached', ['uploadedBy', 'invoiceId']),

  invoices: defineTable({
    sourceReviewedBy: v.optional(v.id('users')),
    sourceReviewedAt: v.optional(v.number()),
    requestId: v.optional(v.string()), requestHash: v.optional(v.string()),
    orgId: v.id("orgs"),
    beneficiaryId: v.id("beneficiaries"),
  ...
  paymentNotifications: defineTable({
    ...
    environment: v.union(v.literal('production'), v.literal('test'), v.literal('unclassified')),
    phase: v.string(),
    ...
    createdAt: v.number(),
    updatedAt: v.number(),
  }).index('by_payment', ['disbursementId']).index('by_series', ['recurringPaymentId']).index('by_org_open', ['orgId', 'environment', 'isOpen', 'updatedAt']),
  paymentNotificationReads: defineTable({
    notificationId: v.id('paymentNotifications'), userId: v.id('users'), revision: v.number(), readAt: v.number(),
  ...
  disbursements: defineTable({
    ...
    executionFee: v.optional(v.object({ token: v.string(), tokenAddress: v.string(), collector: v.string(), amount: v.string() })),
  ...
  delegationReservations: defineTable({ key: v.string(), disbursementId: v.id("disbursements") }).index("by_key", ["key"]),
  ...
  deposits: defineTable({
    ...
    legacyRecord: v.optional(v.object({ amount: v.string(), amountRaw: v.string(), tokenSymbol: v.string(), decimals: v.number(), reconciledAt: v.number() })),
  ...
  outgoingTransfers: defineTable({
    ...
    fromAddress: v.string(), toAddress: v.string(), source: v.literal('safe_tx_service'), createdAt: v.number(),
    paymentRowId: v.optional(v.string()), paymentId: v.optional(v.id('disbursements')),
    reconciliationId: v.optional(v.string()), matchError: v.optional(v.string()),
  }).index('by_org', ['orgId']).index('by_safe_transfer', ['safeId', 'transferId'])
    .index('by_safe_tx', ['safeId', 'txHash']).index('by_payment_row', ['paymentRowId']),
  ...
  depositSyncs: defineTable({
    ...
    scan: v.optional(v.object({ from: v.number(), through: v.number(), cursor: v.string(), page: v.number(), full: v.boolean(), scope: v.optional(v.literal('all')) })),
  ...
  billingCheckouts: defineTable({
    ...
    active: v.boolean(), attemptId: v.optional(v.string()), nonce: v.optional(v.number()), fromBlock: v.optional(v.string()),
    txHash: v.optional(v.string()), replacementHash: v.optional(v.string()), checks: v.number(), error: v.optional(v.string()), recoveryAt: v.optional(v.number()),
    createdAt: v.number(), updatedAt: v.number(),
  }).index('by_org_active', ['orgId', 'active']).index('by_payer_active', ['chainId', 'payer', 'active']).index('by_request', ['orgId', 'requestId']).index('by_recovery', ['recoveryAt']),
  ...
  billing: defineTable({
    orgId: v.id("orgs"),
    trialTier: v.optional(licenseTierValidator), fallbackTier: v.optional(licenseTierValidator),
    licenseGrant: v.optional(licenseGrantValidator), licenseRevision: v.optional(v.number()),
    plan: v.union(
      v.literal("trial"),
      v.literal("starter"),
      v.literal("team"),
  ...
  licenseTiers: defineTable({ name: v.string(), maxUsers: v.union(v.number(), v.null()), maxBeneficiaries: v.union(v.number(), v.null()),
    createdBy: v.id("users"), createdAt: v.number(),
  }),
  licensePrograms: defineTable({ key: v.literal("default"), trialDays: v.number(), trialTier: licenseTierValidator,
    fallbackTier: v.optional(licenseTierValidator), revision: v.number(), updatedBy: v.id("users"), updatedAt: v.number(),
  }).index("by_key", ["key"]),
  licenseEvents: defineTable({ orgId: v.optional(v.id("orgs")), actorUserId: v.id("users"), requestId: v.string(), fingerprint: v.string(),
    action: v.string(), reason: v.string(), before: v.string(), after: v.string(), result: v.string(), createdAt: v.number(),
  }).index("by_request", ["actorUserId", "requestId"]).index("by_org", ["orgId"]),
  ...
  ofacSources: defineTable({
    name: v.literal('ofac_sdn'), activeDatasetId: v.optional(v.id('ofacDatasets')), stagingDatasetId: v.optional(v.id('ofacDatasets')),
    refreshId: v.optional(v.string()), leaseUntil: v.optional(v.number()), lastAttemptAt: v.optional(v.number()),
    lastCheckedAt: v.optional(v.number()), lastError: v.optional(v.string()),
  }).index('by_name', ['name']),
  ofacDatasets: defineTable({
    cleanupAt: v.optional(v.number()), contentsDeletedAt: v.optional(v.number()),
    checksum: v.string(), engine: v.string(), sourceUrl: v.string(), publishedAt: v.number(), fetchedAt: v.number(), activatedAt: v.optional(v.number()), retiredAt: v.optional(v.number()),
    state: v.union(v.literal('staging'), v.literal('active'), v.literal('retired'), v.literal('failed')),
    expectedEntries: v.number(), expectedPostings: v.number(), entryCount: v.number(), postingCount: v.number(), aliasCount: v.number(), addressCount: v.number(),
  }).index('by_state', ['state']).index('by_state_cleanup', ['state', 'cleanupAt']),
  ofacEntries: defineTable({ datasetId: v.id('ofacDatasets'), ...sdnEntryFields }).index('by_dataset_sdn', ['datasetId', 'sdnId']),
  ofacSearchPostings: defineTable({ datasetId: v.id('ofacDatasets'), term: v.string(), part: v.number(), sdnIds: v.array(v.number()) }).index('by_dataset_term', ['datasetId', 'term', 'part']),
  ofacImportChunks: defineTable({ datasetId: v.id('ofacDatasets'), kind: v.union(v.literal('entries'), v.literal('postings')), offset: v.number(), count: v.number(), checksum: v.string() }).index('by_dataset_kind_offset', ['datasetId', 'kind', 'offset']),
  ...
  screeningRuns: defineTable({
    orgId: v.id('orgs'), beneficiaryId: v.id('beneficiaries'), datasetId: v.optional(v.id('ofacDatasets')), engine: v.string(), input: screeningInputValidator, inputFingerprint: v.string(), matchFingerprint: v.string(),
    status: v.union(v.literal('clear'), v.literal('potential_match'), v.literal('unavailable')), matches: v.array(screeningMatchValidator), screenedAt: v.number(), error: v.optional(v.string()),
  }).index('by_recipient', ['beneficiaryId']),
  screeningDecisions: defineTable({
    orgId: v.id('orgs'), beneficiaryId: v.id('beneficiaries'), runId: v.id('screeningRuns'), inputFingerprint: v.string(), matchFingerprint: v.string(),
    status: v.union(v.literal('confirmed_match'), v.literal('false_positive')), reason: v.string(), reviewedBy: v.id('users'), reviewedAt: v.number(), expiresAt: v.number(),
  }).index('by_recipient', ['beneficiaryId']),
  screeningResults: defineTable({
    runId: v.optional(v.id('screeningRuns')), datasetId: v.optional(v.id('ofacDatasets')), engine: v.optional(v.string()), inputFingerprint: v.optional(v.string()), matchFingerprint: v.optional(v.string()),
    decisionId: v.optional(v.id('screeningDecisions')), reviewExpiresAt: v.optional(v.number()), lastError: v.optional(v.string()),
    orgId: v.id("orgs"),
    beneficiaryId: v.id("beneficiaries"),
    status: v.union(
      v.literal("clear"),
