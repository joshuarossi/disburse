...
function isKeySigner(code: string | undefined) {
  return (
    !code ||
    code === "0x" ||
    (/^0xef0100[\da-f]{40}$/i.test(code) && !/0{40}$/.test(code))
  );
}
export async function assertSignatureHandler(
  ...
  if (!slot || slot.length !== 66)
    throw new Error("Could not verify the owning account signature handler");
  ...
  if (
    ...
    !deployments.some((d) => {
      ...
      return (
        (Array.isArray(network) ? network : [network]).some(
          (a) => a?.toLowerCase() === handler.toLowerCase(),
        ) &&
        Object.values(d.deployments).some(
          (p) =>
            p?.address.toLowerCase() === handler.toLowerCase() &&
            p.codeHash === keccak256(code),
        )
      );
    })
  )
    throw new Error(
      "This owning account needs a supported Safe signature handler",
    );
}
...
export async function readAccountAuthority(
  ...
  return readAuthority(chainId, root, atBlock);
}
/** Check a proposed root's complete owner hierarchy before deploying it or
 * transferring a deposit. Contract owners must already support our signatures. */
export async function readProspectiveAccountAuthority(
  chainId: number,
  root: string,
  owners: string[],
  threshold: number,
  ...
  if (
    ...
    throw new Error("Unsupported account approval policy");
  return readAuthority(chainId, root, atBlock, { owners, threshold });
}
async function readAuthority(
  chainId: number,
  root: string,
  atBlock?: bigint,
  initial?: { owners: string[]; threshold: number },
): Promise<AccountAuthority> {
  const client = getChainClient(chainId);
  if ((await client.getChainId()) !== chainId)
    throw new Error("The account reader returned another network");
  ...
  const visit = async (raw: string, ancestors: string[]) => {
    ...
    if (ancestors.includes(address))
      throw new Error("The account approval hierarchy contains a cycle");
    if (ancestors.length > 3)
      throw new Error(
        "Account approvals support at most three levels of owning accounts",
      );
    if (nodes.has(address)) return;
    if (nodes.size >= 32)
      throw new Error("The account approval hierarchy exceeds 32 accounts");
    const proposed = initial && !ancestors.length;
    if (!proposed)
      await assertSafeIdentity(client, address, chainId, blockNumber);
    if (ancestors.length)
      await assertSignatureHandler(client, address, chainId, blockNumber);
    const [owners, threshold, nonce] = proposed
      ? ([initial.owners as Address[], BigInt(initial.threshold), 0n] as const)
      : await Promise.all([
          client.readContract({
            address,
            abi: authorityAbi,
...
export function approvalPaths(
  ...
  const visit = (address: string, path: string[]) => {
    ...
    if (
      node.owners.includes(wallet.toLowerCase()) &&
      !node.contracts.includes(wallet.toLowerCase())
    )
      paths.push(next);
...
export function availableAccountApprovals(
  authority: AccountAuthority,
  wallets: string[],
) {
  ...
  const visit = (address: string, depth: number): boolean => {
    if (++visits > 128 || depth > 3)
      throw new Error("Unsupported account approval hierarchy");
    const node = authority.nodes.find((n) => n.address === address)!;
    return (
      node.owners.filter((owner) =>
        node.contracts.includes(owner)
          ? visit(owner, depth + 1)
          : approved.has(owner),
      ).length >= node.threshold
    );
  };
