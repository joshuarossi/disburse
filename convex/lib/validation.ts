...
// These run inside Convex mutations/queries/actions — never trust client input.
...
export function assertValidAddress(value: unknown, label = "address"): asserts value is string {
  if (!isValidAddress(value)) {
    throw new Error(`Invalid ${label}: expected a 20-byte hex address (0x + 40 hex chars)`);
  }
}
...
export function assertValidTxHash(value: unknown, label = "tx hash"): asserts value is string {
  if (!isValidTxHash(value)) {
    throw new Error(`Invalid ${label}: expected a 32-byte hex hash (0x + 64 hex chars)`);
  }
}
