...
export const context = internalQuery({
  ...
  handler: async (ctx, args) => {
    const expected = await verificationContext(ctx, {
      ...args,
      candidateHash:
        (await ctx.db.get(args.disbursementId))?.safeTxHash ?? emptyHash,
    });
    ...
    return {
      ...
      accountNames: accounts
        .filter((a) => a.chainId === expected.chainId)
        .map((a) => ({
...
export const recoverOriginal = action({
  ...
  handler: async (ctx, args): Promise<string> => {
    ...
    if (
      !source.payment.safeTxHash ||
      !["pending", "proposed", "scheduled"].includes(source.payment.status)
    )
      throw new Error("No original approval is available for recovery");
    const original = source.original?.proposal;
    const tx = original
      ? {
          ...original.safeTransactionData,
          safe: original.safeAddress,
          confirmations: [
            {
              owner: original.senderAddress,
              signature: original.senderSignature,
            },
          ],
        }
      : await loadPaymentProposal(ctx, args.disbursementId, source.expected);
    await assertSafeProposal(tx, source.expected, false);
    const approvals = await readOwnerApprovalStatus(
      tx,
      source.expected.chainId,
      source.expected.safeAddress,
      source.payment.safeTxHash as `0x${string}`,
    );
    if (approvals.currentNonce > approvals.proposalNonce)
      throw new Error(
        "Check settlement of the original transaction before recovering its approvals",
      );
    const signatures = (tx.confirmations ?? []).filter(
      (s) =>
        !s.isContractSignature &&
        approvals.confirmedOwners.includes(s.owner.toLowerCase()),
    );
    const proposal: PreparedOwnerProposal = {
      safeAddress: tx.safe,
      safeTxHash: source.payment.safeTxHash,
      senderAddress: source.expected.actorWallet!,
      senderSignature: signatures[0]?.signature ?? "0x",
      safeTransactionData: {
        to: tx.to,
        value: tx.value,
        data: tx.data ?? "0x",
    ...
    return ctx.runMutation(internal.accountApprovals.importOriginal, {
      ...args,
      proposal,
      signatures,
      snapshot: source.expected.snapshot,
    });
  },
});
export const importOriginal = internalMutation({
  args: {
    ...
    signatures: v.array(
      v.object({
        owner: v.string(),
        signature: v.string(),
        isContractSignature: v.optional(v.boolean()),
      }),
    ),
    snapshot: v.string(),
  },
  handler: async (ctx, args) => {
    ...
    if (
      current.snapshot !== args.snapshot ||
      args.proposal.safeTxHash !== payment.safeTxHash
    )
      throw new Error("The original payment changed during recovery");
    ...
    const conflict = await ctx.db
      ...
      .withIndex("by_account_nonce", (q) =>
        q
          ...
          .eq("nonce", args.proposal.safeTransactionData.nonce),
      )
      .first();
    if (conflict)
      throw new Error(
        "Another saved payment uses this account transaction number",
      );
    ...
    await ctx.db.patch(payment._id, {
      ...
      preparedProposalAt: payment.preparedProposalAt ?? Date.now(),
      updatedAt: Date.now(),
    });
    await appendAudit(ctx, {
      orgId: payment.orgId,
      actorUserId: user._id,
      action: "disbursement.approvals_recovered",
      objectType: "disbursement",
      objectId: payment._id,
      metadata: { safeTxHash: payment.safeTxHash, signatures: seen.size },
    });
...
export const forSigning = action({
  ...
  handler: async (ctx, args): Promise<SigningRequest> => {
    ...
    const source = await ctx.runQuery(internal.accountApprovals.context, args);
    if (
      !["draft", "pending", "proposed", "scheduled"].includes(
        source.payment.status,
      ) ||
      source.payment.allowanceExecution
    )
      throw new Error("This payment cannot accept approvals");
    if (source.payment.safeTxHash && !source.saved)
      throw new Error("Resume this payment through its original approval flow");
    ...
    const tx =
      source.saved?.proposal.safeTransactionData ??
      prepareAccountTransaction(source.expected, nonce);
    const proposal = source.saved?.proposal ?? {
      safeAddress: source.expected.safeAddress,
      safeTxHash: approvalSigningData(
        source.expected.chainId,
        [authority.root],
        tx,
      ).hash,
    ...
    if (root.nonce > tx.nonce)
      throw new Error(
        "This account transaction number has already been used. Check the original payment settlement.",
      );
    await assertSafeProposal(
      { ...tx, safe: proposal.safeAddress },
      { ...source.expected, safeTxHash: proposal.safeTxHash },
      false,
    );
    const paths = approvalPaths(authority, source.expected.actorWallet!).map(
      (path) => ({
        path,
        labels: path.map(
          (a) =>
        ...
        approved: source.signatures.some(
          (s) =>
            s.owner === source.expected.actorWallet!.toLowerCase() &&
            s.pathKey === path.join(":"),
        ),
      }),
    );
    if (!paths.length)
      throw new Error(
        "Your wallet is not a current approver for this account or its owning accounts",
      );
...
export const save = action({
  ...
  handler: async (ctx, args): Promise<string> => {
    for (let attempt = 0; attempt < 2; attempt++) {
      const source = await ctx.runQuery(internal.accountApprovals.context, {
      ...
      if (
        source.saved &&
        source.saved.proposal.safeTxHash.toLowerCase() !==
          args.proposal.safeTxHash.toLowerCase()
      )
        throw new Error("Resume the original saved payment");
      const authority = await readAccountAuthority(
        expected.chainId,
        expected.safeAddress,
      );
      if (authority.nodes[0].nonce > args.proposal.safeTransactionData.nonce)
        throw new Error(
          "This account transaction number has already been used",
        );
      if (
        !source.saved &&
        args.proposal.safeTransactionData.nonce !==
          Math.max(authority.nodes[0].nonce, source.latestNonce + 1)
      )
        throw new Error(
          "The account queue changed. Review the payment again before signing.",
        );
      await assertSafeProposal(
        {
          ...args.proposal.safeTransactionData,
          safe: args.proposal.safeAddress,
        },
        { ...expected, safeTxHash: args.proposal.safeTxHash },
        false,
      );
      const digest = await verifyAccountSignature(
        ...
        {
          path: args.path,
          owner: expected.actorWallet!,
          signature: args.signature,
        },
      );
      const stored = await ctx.runMutation(internal.accountApprovals.persist, {
        ...args,
        digest,
        snapshot: expected.snapshot,
      });
      if (typeof stored === "string") return stored;
      // Another owner's first approval may have saved the same proposal. Re-read
      // and reverify the exact signed intent, membership and current chain authority.
      // No wallet prompt or replacement authorization is created by this retry.
    }
    throw new Error("Payment details changed during approval. Review again.");
  },
});
export const persist = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await assertMemberPaymentPolicy(
      ...
      payment.scheduledAt ?? payment.createdAt,
      payment._id,
    );
    const existing = await ctx.db
      .query("accountProposals")
    ...
    const original = await ctx.db
      .query("accountSignatures")
      .withIndex("by_payment_signer", (q) =>
        q
          .eq("disbursementId", payment._id)
          .eq("pathKey", pathKey)
          .eq("owner", owner),
      )
      .unique();
    if (
      original &&
      existing?.proposal.safeTxHash.toLowerCase() ===
        args.proposal.safeTxHash.toLowerCase() &&
      original.digest === args.digest
    )
      return existing.proposal.safeTxHash;
    const current = await verificationContext(ctx, {
      ...args,
      candidateHash: args.proposal.safeTxHash,
    });
    if (current.snapshot !== args.snapshot) return { changed: true as const };
    if (
      existing &&
      existing.proposal.safeTxHash.toLowerCase() !==
        args.proposal.safeTxHash.toLowerCase()
    )
      throw new Error("The original payment approval cannot be replaced");
    if (!existing) {
      if (payment.safeTxHash)
        throw new Error("This payment already has a saved proposal");
      ...
      if (conflict)
        throw new Error(
          "Another payment reserved this account transaction number. Review again.",
        );
      await ctx.db.insert("accountProposals", {
        disbursementId: payment._id,
        accountKey,
        ...
        proposal: {
          ...args.proposal,
          senderAddress: owner,
          senderSignature: args.signature,
        },
        createdAt: Date.now(),
      });
      await ctx.db.patch(payment._id, {
    ...
    if (
      ...
      throw new Error("This payment has reached its approval evidence limit");
    await ctx.db.insert("accountSignatures", {
      disbursementId: payment._id,
      pathKey,
      path,
    ...
    await appendAudit(ctx, {
      orgId: payment.orgId,
      actorUserId: user._id,
      action: "disbursement.account_approval",
      objectType: "disbursement",
      objectId: payment._id,
      metadata: {
        safeTxHash: args.proposal.safeTxHash,
...
export const execution = action({
  ...
  handler: async (ctx, args): Promise<{ to: string; data: string }> => {
    ...
    const proposal = await loadPaymentProposal(
      ctx,
      args.disbursementId,
      expected,
    );
    ...
    return {
      to: expected.safeAddress,
      data: encodeExecTransaction({
        ...proposal,
        data: proposal.data ?? undefined,
        refundReceiver: proposal.refundReceiver ?? undefined,
      }),
    };
  },
});
