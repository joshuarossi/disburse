// Live Base Sepolia company subscription checkout. No native-gas submission.
...
const run = option("run"),
  action = ["prepare", "execute", "status", "resume-approvals"].filter((v) =>
...
const directory = ".local/qa/circle-billing";
...
const fresh = ["prepare"].includes(action[0]);
if (fresh === !!saved)
  throw new Error(
...
try {
  ...
  if (action[0] === "prepare") {
    const billingCheckoutId = await client.mutation(
      api.billingCheckoutData.create,
      {
    ...
    await save({ source: { billingCheckoutId } });
    ...
    const checkout = await client.query(api.billingCheckoutData.get, {
      orgId: previous.orgId,
      checkoutId: saved.source.billingCheckoutId,
      sessionToken,
    });
