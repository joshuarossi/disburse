// Live Base Sepolia nested company account setup. No native-gas submission.
...
const directory = ".local/qa/circle-account";
...
try {
  ...
  if (action[0] === "prepare") {
    const accountSetupId = await client.action(api.accountSetups.create, {
      orgId: previous.orgId,
      parentSafeId: previous.safeId,
      name: saved.name,
      requestId: saved.requestId,
      sessionToken,
    });
    await save({ source: { accountSetupId } });
    ...
    const account = await client.query(api.accountSetups.get, {
      ...saved.source,
      sessionToken,
    });
