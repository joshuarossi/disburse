...
The v2 financial workspace supports English, Spanish and Brazilian Portuguese. Settings and profile preferences expose a language selector. Onboarding, customer invoice pages, recipient detail collection, and the public site use the same preference. Localized number/date displays preserve exact amounts and UTC scheduling; stored records and machine exports retain their original values. See [localization](LOCALIZATION.md).
