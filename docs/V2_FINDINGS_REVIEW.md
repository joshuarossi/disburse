...
**Confirmed and fixed.** Matching `ExecutionFailure` is useful for locating the original transaction, but its outcome must be interpreted separately. After confirmation depth and the canonical block hash are checked, an `ExecutionFailure` emitted by the original Safe for the original Safe transaction hash finalizes the payment as `failed`, retains the network transaction hash, stops automatic recovery and records an audit entry. Native and historical relay recovery both apply these checks. An unverified provider hash cannot pin the payment to an unrelated receipt. Failure evidence is separate from a successful accounting settlement.
...
**Subsequent service and QA work:** the [September 7 report](CUSTOMER_PAID_SERVICES_QA_2026-09-07.md) supersedes the verification counts above. The customer must pay all service costs directly in stablecoins, with no gas funding or external provider bill charged to Disburse. Turbo and outgoing Resend are disabled; private invitation sharing replaces paid delivery. Circle/Candide has three actual customer-paid testnet receipts, including failure and recovery, while the original onboarding provider still rejects live submission. New Safe creation, nested handler verification, error presentation and durable setup recovery have been updated. Complete stablecoin-fee application integration remains unfinished. The [requirements](PRODUCT_AND_SERVICE_REQUIREMENTS.md) and corrected [provider guide](GELATO_V2_SETUP.md) cover the remaining implementation work. The signed-allowance abandonment constraint in item 1 remains explicit.
