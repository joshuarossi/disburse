...
The release adds contract-based spending allowances, customer-paid managed fees, durable payment recovery, unique invoice receiving addresses with customer-paid USDC collection, credit notes and refunds, and reviewed accounting exports. Treasury services include direct Aave lending, Uniswap conversion and Circle CCTP transfers between company accounts. Operator-managed trials, complimentary grants and reusable free tiers keep SaaS access separate from fund ownership. Customers pay all network and provider fees.
...
This candidate has passed development and Sepolia acceptance described in [QA evidence](QA_V2.md). Actual USDC-fee execution, unattended scheduling, Team activation/renewal, collections, refunds and treasury provider integrations have testnet receipts. A complete built-app finance cycle with two parent owners now reaches a balanced journal export. Live Pro upgrade credit, MetaMask mainnet setup, extension/mobile connectors and external-ledger acceptance remain separate checks. Mainnet invoice issuance stays disabled pending independent contract review. The finance workspace, onboarding, and public customer/recipient flows support English, Spanish and Brazilian Portuguese. Language changes preserve form state and the same local/account preference is used throughout. See [launch readiness](LAUNCH_READINESS.md), [deployment](DEPLOYMENT.md) and the [active program](../TODOS.md).
