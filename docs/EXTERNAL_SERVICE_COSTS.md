...
| Transfers between networks                     | Circle CCTP Forwarding Service                    | The customer pays delivery from the gross USDC transfer and execution separately in USDC. No application key, prepaid balance or operator usage bill. See [account transfers](ACCOUNT_TRANSFERS.md). |
| Yield and token swaps                           | Pending provider integrations                    | No paid API, strategy or replacement service is activated by this inventory. Each new adapter must meet the same customer-cost rule. |
