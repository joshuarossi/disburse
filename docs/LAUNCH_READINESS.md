...
| Accounting | Verified movements, reviewed functional-currency amounts and book references, chart imports, balanced journals, credits/refunds, immutable exports, corrections and closed periods | Actual Sepolia movements reconcile to historical balances. Synthetic book acceptance does not establish an external-ledger import or an accountant-approved classification. The customer's ledger remains the book of record. |
...
Current checks pass 1,187 unit/backend tests, typecheck, lint, receiving-contract tests, release-configuration tests and the build. All 395 stories pass in the latest full browser run. A previous committed build passed GitHub CI and Cloudflare Pages, and its actual sign-in page was opened in Chromium. These are separate from production payment acceptance.

Live evidence and scope: [customer-paid services](CUSTOMER_PAID_SERVICES_QA_2026-09-07.md), [delegated payments](DELEGATED_PAYMENTS.md), [receivables](ACCOUNTS_RECEIVABLE.md), [account transfers](ACCOUNT_TRANSFERS.md), [lending](LENDING.md), [conversions](CONVERSIONS.md), [receiving costs](RECEIVING_COSTS.md), [operations rehearsal](OPERATIONS_REHEARSAL.md).

## Remaining implementation and acceptance

The active work list is [TODOS.md](../TODOS.md). The complete second-approver finance cycle, final localized copy and built-screen visual review remain in progress. No requirement is marked complete solely because its fixtures pass.
...
Continue the remaining coding and testnet acceptance in the draft PR. Public launch should follow the production-specific evidence above; a passing CI badge cannot stand in for it. The deployment procedure and rollback limits are in [DEPLOYMENT.md](DEPLOYMENT.md).
