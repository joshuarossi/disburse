...
export function assertPaymentIntent(
  ...
  const actual = calls
    .map((c) => {
      if (
        c.operation !== 0 ||
        BigInt(c.value) !== 0n ||
        ![expected.tokenAddress, expected.executionFee?.tokenAddress].some(a => a?.toLowerCase() === c.to.toLowerCase())
      )
      ...
      return `${c.to.toLowerCase()}:${decoded.args[0].toLowerCase()}:${decoded.args[1]}`;
    })
    .sort();
  ...
  if (expected.executionFee) {
    ...
    intended.push(`${fee.tokenAddress.toLowerCase()}:${fee.collector.toLowerCase()}:${amountToBaseUnits(fee.amount, fee.token)}`);
    intended.sort();
  }
  if (
    !intended.length ||
