...
function AccountingWorkspace({
  ...
  const scope =
    access && environment !== "unclassified"
      ? { ...access, environment }
      : null;
  const config = useQuery(api.accounting.configuration, access ?? "skip");
  const [view, setView] = useState<
    "activity" | "receipts" | "journals" | "exports" | "balances"
  >("activity");
  const [settings, setSettings] = useState(false),
    [source, setSource] = useState<AccountingSource | null>(null);
  const [exportId, setExportId] = useState<Id<"accountingExports"> | null>(
    null,
  );
  ...
  const activity = useQuery(
    ...
    scope && view === "activity"
      ? { ...scope, pageSize: 50, cursor: pages.cursor }
      : "skip",
  );
  const receipts = useQuery(
    api.accounting.listReceipts,
    scope && view === "receipts" ? { ...scope, cursor: pages.cursor } : "skip",
  );
  const journals = useQuery(
    api.accounting.listEntries,
    scope && view === "journals" ? { ...scope, cursor: pages.cursor } : "skip",
  );
  const exports = useQuery(
    api.accounting.listExports,
    scope && view === "exports" ? { ...scope, cursor: pages.cursor } : "skip",
  );
  const page =
    view === "activity"
      ? activity
  ...
  const chooseExport = async () => {
    ...
    try {
      ...
      setError(userErrorMessage(e, "Could not prepare this export"));
    } finally {
      setBusy(false);
    }
  };
                      ...
                      {activityRows.map((row) => (
                        <li key={row.rowId} className="space-y-3 p-4">
                          ...
                          <button
                            className="workspace-button"
                            onClick={() =>
                              setSource({ kind: "activity", id: row.rowId })
                            }
                          >
                          ...
                          {activityRows.map((row) => (
                            <tr key={row.rowId}>
                              ...
                              <td>
                                ...
                                <span className="workspace-table-secondary font-mono text-xs">
                                  {row.accountAddress.slice(0, 8)}…
                                  {row.accountAddress.slice(-6)}
                                </span>
                              </td>
                      ...
                      {receipts.page.map((row) => (
                        <li key={row.id} className="space-y-3 p-4">
                          ...
                          <button
                            className="workspace-button"
                            onClick={() =>
                              setSource({ kind: "receipt", id: row.id })
                            }
                          >
                            {row.state
                              ? "View reconciliation"
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  onClick={() => {
                    pages.next(page.continueCursor);
                    setSelected([]);
                  }}
                >
                  Next
