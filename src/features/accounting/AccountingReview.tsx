    ...
    <div className="space-y-5 p-5 sm:p-6">
      {!details || !config ? <LoadingRows /> : details.error || !details.fact ? <>
        ...
        : !config.profile ? <Notice tone="info">Set up your book and chart of accounts in Reports → Reconciliation first.</Notice> : <>
          <section className="rounded-xl border border-white/10 p-4 space-y-2" aria-label="Settled movement">
            ...
            <p className="text-lg font-semibold tabular-nums">{details.fact.amount} {details.fact.token}
              <span className="ml-2 text-xs font-normal text-slate-400">{details.fact.direction === 'inflow' ? 'Received' : 'Sent'}</span></p>
            <p className="text-sm">{details.fact.accountName} · {getChainName(details.fact.chainId)}</p>
            <p className="text-xs text-slate-400">Settled {new Date(details.fact.settledAt).toISOString().replace('T', ' ').slice(0, 19)} UTC</p>
            ...
            <details className="text-xs text-slate-400"><summary className="cursor-pointer">Settlement evidence</summary>
...
function ReviewForm({ orgId, details, config, onSaved }: { orgId: Id<'orgs'>; details: Details; config: Configuration; onSaved: () => void }) {
  ...
  const [counterId, setCounterId] = useState(previous?.lines[1]?.account.id ?? '');
  ...
  try {
    ...
    if (treatment === 'already_recorded') bookUnits(assetValue, profile.currency, true);
      ...
      differenceAccount: differenceId ? account(differenceId) : undefined, externalName, companyTransfer: fact.companyTransfer, lendingMovement: fact.lendingMovement, conversionMovement: fact.conversionMovement,
  ...
  const options = (Object.keys(accountingTreatments) as AccountingTreatment[]).filter(value => value === 'already_recorded'
    || (fact.conversionMovement ? value === 'currency_conversion' : fact.lendingMovement ? value === (fact.lendingMovement === 'supply' ? 'investment_deposit' : 'investment_withdrawal') : fact.companyTransfer ? value === 'internal_transfer'
  ...
  return <form className="space-y-5" onSubmit={async e => {
    ...
    try {
      await save({ orgId, sessionToken, source: fact.source, expectedFingerprint: fact.fingerprint, expectedProfileVersion: profile.version,
        treatment, postingDate, assetBookValue: assetValue, obligationBookValue: obligation || withdrawal || conversionReceipt ? obligationValue : undefined,
    ...
    {treatment && treatment !== 'already_recorded' && <div className="grid gap-4 sm:grid-cols-2">
      {selectAccount('Holding account in your books', assetId, setAssetId, ['asset'])}
