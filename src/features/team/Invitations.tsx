...
export function Invitations({
  ...
  const [shared, setShared] = useState<{ id: string; url: string } | null>(
    null,
  );
  ...
  return (
    <section className="workspace-panel" aria-label={tx("Team invitations")}>
      <div className="workspace-toolbar">
        <div>
          <h2 className="font-semibold">{tx("Invitations")}</h2>
      ...
      <div className="p-5 space-y-4">
        ...
        {rows === undefined ? (
          ...
          rows.map((row) => (
            <article
              ...
              aria-label={tx("Invitation for {{value1}}", {
                value1: row.email,
              })}
            >
              <div className="flex flex-wrap justify-between gap-3">
                ...
                <div className="text-sm sm:text-right">
                  ...
                  <p className="workspace-table-secondary">
                    {tx(deliveryLabels[row.deliveryStatus])}
                  </p>
                </div>
              </div>
              <p className="workspace-table-secondary">
                {tx("Created")} {formatDate(row.createdAt)} ·{" "}
                {row.status === "accepted" && row.acceptedAt
                  ? tx("Accepted {{value1}}", {
                      value1: formatDate(row.acceptedAt),
                    })
                  : tx("Expires {{value1}}", {
                      value1: formatDate(row.expiresAt),
                    })}
              </p>
              {row.deliveryError && (
                <p className="text-sm workspace-funding-warning">
                  {userErrorMessage(
                    row.deliveryError,
                    "The earlier email delivery could not be confirmed.",
                  )}
                </p>
              )}
              {isAdmin && row.status !== "accepted" && (
                <div className="flex flex-wrap gap-2">
                  {row.status === "pending" &&
                    row.deliveryStatus === "ready_to_share" && (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            setShared({ id: row.id, url: result.url });
                          } catch (error) {
                            setError(
                              userErrorMessage(
                                error,
                                "The invitation link could not be opened. Try again.",
                              ),
                            );
                          } finally {
                            setBusy("");
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setError(
                          userErrorMessage(
                            e,
                            "The invitation link could not be replaced.",
                          ),
                        );
                      } finally {
                        setBusy("");
                  ...
                  {["pending", "unavailable"].includes(row.status) && (
                    ...
                    >
                      {tx("Revoke invitation")}
                    </button>
                  )}
                </div>
              )}
              {shared?.id === row.id && row.status === "pending" && (
                <InvitationLink url={shared.url} email={row.email} />
              )}
              {confirm === row.id && (
                <div className="space-y-3 border-t border-white/10 pt-3">
                  <p className="text-sm">
                    {tx("Revoke the private link for")} {row.email}
                    {tx("? They will need a new invitation to join.")}
                  </p>
        ...
        {!!rows?.length && (
          <p className="workspace-table-secondary">
            {tx("Showing the latest")} {rows.length}{" "}
            {tx(
              "invitations. Invitations for a known sign-in wallet appear under Members. A shared link does not confirm that the invitation has been read or accepted.",
            )}
          </p>
        )}
      </div>
