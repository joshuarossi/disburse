...
describe('Safe allowance policy proposals', () => {
  ...
  it('does not guess module addresses on unsupported networks', () => {
    expect(getAllowanceDeployments(84532)).toEqual([]);
    expect(getAllowanceDeployments(42161).map(d => d.version)).toEqual(['1.0.0', '0.1.1']);
  ...
  it('allows revocation of legacy grants but refuses new authority on them', () => {
    for (const legacy of getAllowanceDeployments(137).filter(d => d.legacy)) {
      expect(() => buildAllowanceGrant({ ...grant, chainId: 137, module: legacy.address })).toThrow('outdated');
      expect(buildAllowanceRevocation(137, legacy.address, delegate, safe)).toHaveLength(1);
    }
  });
});
