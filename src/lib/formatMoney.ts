import { tx, workspaceLocale } from "./workspaceI18n";
...
export function formatMoney(
  ...
  if (!match) return tx("Unavailable");
  ...
  fraction = precise
    ...
    : fraction.slice(0, 2);
  const separator =
    new Intl.NumberFormat(workspaceLocale())
      .formatToParts(1.1)
      .find((p) => p.type === "decimal")?.value ?? ".";
  const prefix = token === "EURC" ? "€" : "$";
  return `${negative && rounded !== 0n ? "-" : ""}${prefix}${new Intl.NumberFormat(workspaceLocale()).format(integer)}${separator}${fraction}`;
}
...
export function formatAssetAmount(
  ...
  if (!match) return tx("Unavailable");
  const integer = new Intl.NumberFormat(workspaceLocale()).format(
    BigInt(match[2]),
  );
...
export const scheduleDateTime = (at: number) =>
  `${new Date(at).toLocaleString(workspaceLocale(), { dateStyle: "medium", timeStyle: "short", timeZone: "UTC" })} UTC`;
