...
for (const [theme, width] of [['light', 1440], ['dark', 430]] as const) {
  test(`a nested spending policy explains each approval and the fee in ${theme}`, async ({ page }) => {
    await page.setViewportSize({ width, height: 1000 });
    await page.addInitScript(theme => { localStorage.setItem('theme', theme); sessionStorage.setItem('qa:scenario', 'policy-nested'); }, theme);
    ...
    await expect(queue.getByRole('button', { name: 'Apply policy' })).toBeDisabled();
    ...
    await page.screenshot({ path: `.local/qa/policy-approval-${theme}.png`, fullPage: true });
...
test('changed policy state blocks approval and a declined send keeps its original retry', async ({ page }) => {
  ...
  await expect(queue.getByRole('button', { name: 'Approve policy' })).toHaveCount(0);
  await page.evaluate(() => sessionStorage.setItem('qa:scenario', 'policy-declined'));
  await page.reload();
  await page.getByRole('tab', { name: 'Delegated spending' }).click();
  await expect(queue).toContainText('Ready to retry');
  await queue.getByRole('checkbox').check();
  await expect(queue.getByRole('button', { name: 'Retry original policy' })).toBeEnabled();
  await expect(queue.getByRole('button', { name: 'Check policy confirmation' })).toBeEnabled();
});

test('the keyboard skip link stays hidden during pointer use and appears on focus', async ({ page }) => {
  await page.goto('/org/demo/team');
...
test('archived accounts retain allowance inspection and revocation without new grants', async ({ page }) => {
  await page.addInitScript(() => sessionStorage.setItem('qa:scenario', 'policy-archived'));
  await page.goto('/org/demo/team');
  await page.getByRole('tab', { name: 'Delegated spending' }).click();
  await expect(page.getByRole('combobox', { name: 'Funding account', exact: true })).toContainText('Archived');
  await expect(page.getByText('Existing allowances can still authorize transfers.', { exact: false })).toBeVisible();
